...
def read_files_split(df):
    ...
    n_e_dic = {
        ...
        '49': [118.94487719912863, 32.04948002478078], '65': [118.9440911184693, 32.04922271168032],
                        ...
                        for tmp_id in unique_ids:  # 检测所有传感器
                            if (sensor_dfs[tmp_id]['Audio'] >= 100).any() or (
                                ...
                                target_mblb_tmp.update(
                                  ...
                                  70 < sensor_dfs[tmp_id]['Audio'].mean() < 100) or \
...
if button5:
    ...
    fir_cyc_num = 1
    while True:
        print(f'第{fir_cyc_num}次执行Button5第一层循环')
        fir_cyc_num += 1
        # st.markdown(button2)
        if button1 or button2 or button3 or button4 or button6:
            ...
            if first_elapsed_time >= 30:
                ...
                cyc_num = 1
                while True:
                    print(f'第{cyc_num}次执行Button5第二层循环')
                    cyc_num += 1
                    elapsed_time = time.time() - start_time2
                    # print(elapsed_time)
                    if elapsed_time >= 5:
...
if button6:
    ...
    fir_cyc_num = 1
    while True:
        print(f'第{fir_cyc_num}次执行Button6第一层循环')
        fir_cyc_num += 1
        # st.markdown(button2)
        if button1 or button2 or button3 or button4 or button5:
            ...
            if first_elapsed_time >= 30:
                ...
                cyc_num = 1
                while True:
                    print(f'第{cyc_num}次执行Button6第二层循环')
                    cyc_num += 1
                    elapsed_time = time.time() - start_time2
                    # print(elapsed_time)
                    if elapsed_time >= 5:
